...
- For an unintended config change, run `./scripts/monaco/Invoke-MonacoDownload.ps1 -Path . -Environment <env> -Output downloaded/<env>` to snapshot the current state, diff against the pre-deploy commit, and prepare a revert PR through normal review.
...
1. Locally: `./scripts/monaco/Sync-ConfigCatalog.ps1` (no `-Check`) to regenerate.
