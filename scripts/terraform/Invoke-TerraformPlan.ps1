...
# Translate dt-pilot canonical env vars to provider-specific names so
# the child terraform process can authenticate.
...
$planResult = Invoke-TerraformCommand -TerraformExe $exe -Arguments $args -WorkingDirectory $workDir -CaptureOutput
...
$planBinPath  = if ([System.IO.Path]::IsPathRooted($Out)) { $Out } else { (Join-Path $workDir $Out) }
