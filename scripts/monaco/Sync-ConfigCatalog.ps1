...
.EXAMPLE
    ./scripts/monaco/Sync-ConfigCatalog.ps1

.EXAMPLE
    ./scripts/monaco/Sync-ConfigCatalog.ps1 -Check
#>
...
# Moved to scripts/monaco/ in the multi-backend skeleton PR, so the repo
# root is two parents up (one out of monaco/, one out of scripts/).
# Other repo-wide scripts at scripts/ root only need one Split-Path.
...
function New-ScaffoldFiles {
    ...
    # SCAFFOLD.md
    $scaffoldMd = @()
    $scaffoldMd += "<!-- GENERATED FILE - do not hand-edit. Regenerate with ./scripts/monaco/Sync-ConfigCatalog.ps1 -->"
    ...
    $scaffoldMd += "   ./scripts/monaco/Invoke-MonacoGenerate.ps1 -Path . -Type schema -Schema $($Entry.id)"
    $scaffoldMd += '   ```'
    $scaffoldMd += ""
    ...
    # config.yaml.example
    $configYaml = @()
    $configYaml += "# GENERATED FILE - do not hand-edit. Regenerate with ./scripts/monaco/Sync-ConfigCatalog.ps1"
    ...
    $jsonEntries.Add('  "_comment": "GENERATED FILE - do not hand-edit. Regenerate with ./scripts/monaco/Sync-ConfigCatalog.ps1. Replace this with the real Dynatrace payload; run monaco generate schema for the authoritative shape."')
    if ($params.Count -gt 0) {
        foreach ($p in $params) {
...
if ($Check) {
    ...
    if ($drift.Count -gt 0) {
        ...
        Write-Host ""
        Write-Host "Regenerate locally with: ./scripts/monaco/Sync-ConfigCatalog.ps1" -ForegroundColor Yellow
