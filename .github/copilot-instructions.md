...
> **Wrapper script paths.** Monaco wrappers live under [`scripts/monaco/`](../scripts/monaco/) (e.g. `scripts/monaco/Invoke-MonacoDryRun.ps1`). Bare-name references like `Invoke-MonacoDryRun.ps1` in this file mean that file. See [`CLAUDE.md`](../CLAUDE.md) Backend Routing for the full mapping and [`config/catalog/backends.json`](../config/catalog/backends.json) for the registry.
...
- **Use the wrapper scripts in `scripts/monaco/`** instead of typing `monaco` commands directly.
